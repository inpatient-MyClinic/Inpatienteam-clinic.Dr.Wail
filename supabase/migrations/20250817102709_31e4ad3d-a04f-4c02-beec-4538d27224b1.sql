-- Fix security issues from the analytics functions

-- Drop existing functions and recreate with proper search paths
DROP FUNCTION IF EXISTS get_monthly_branch_stats(int, int);
DROP FUNCTION IF EXISTS get_monthly_hospital_stats(int, int);
DROP FUNCTION IF EXISTS get_monthly_top_specialties(int, int);
DROP FUNCTION IF EXISTS get_monthly_conversion_trends(int);
DROP FUNCTION IF EXISTS get_monthly_dashboard_data(int, int);
DROP FUNCTION IF EXISTS refresh_monthly_analytics();

-- Revoke access to materialized view and drop it
REVOKE SELECT ON monthly_analytics_cache FROM authenticated;
DROP MATERIALIZED VIEW IF EXISTS monthly_analytics_cache;
