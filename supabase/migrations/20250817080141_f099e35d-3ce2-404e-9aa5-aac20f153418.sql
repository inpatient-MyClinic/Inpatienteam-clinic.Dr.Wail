...
-- 1. Create audit log table for tracking all sensitive data access
...
EXCEPTION 
  WHEN OTHERS THEN
    -- Log errors but don't block access
    NULL;
END;
$$;
...
-- 6. Secure messages table - restrict access to intended recipients only
...
DROP POLICY IF EXISTS "Finance users can manage transactions" ON public.finance_transactions;
DROP POLICY IF EXISTS "Finance secure access" ON public.finance_transactions;
DROP POLICY IF EXISTS "Request owners can view their transaction status only" ON public.finance_transactions;
