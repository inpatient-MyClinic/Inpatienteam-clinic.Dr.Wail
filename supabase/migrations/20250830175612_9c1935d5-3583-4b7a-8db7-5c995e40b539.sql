-- Fix the analytics function to use correct field mappings
...
BEGIN
  ...
  -- Updated to use correct field mappings: Date instead of Date of Request:, Status instead of Status of operation
  WITH deduplicated_data AS (
    SELECT DISTINCT ON (
      COALESCE(raw_data->>'Patient Name', raw_data->>'Patient''s Name:'), 
      COALESCE(raw_data->>'Date', raw_data->>'Date of Request:'), 
      COALESCE(raw_data->>'Status', raw_data->>'Status of operation'),
      COALESCE(raw_data->>'Hospital Name', raw_data->>'Referred Hospital')
    )
      COALESCE(raw_data->>'Status', raw_data->>'Status of operation') as status_value,
      COALESCE(raw_data->>'Branch', raw_data->>'My Clinic Branch') as branch_value,
      COALESCE(raw_data->>'Hospital Name', raw_data->>'Referred Hospital') as hospital_name,
      raw_data->>'Specialty' as specialty
    FROM excel_rows_raw err
    WHERE (raw_data->>'Status' IS NOT NULL OR raw_data->>'Status of operation' IS NOT NULL)
      AND (raw_data->>'Date' IS NOT NULL OR raw_data->>'Date of Request:' IS NOT NULL)
      AND (raw_data->>'Date' != '' OR raw_data->>'Date of Request:' != '')
      AND parse_excel_date(COALESCE(raw_data->>'Date', raw_data->>'Date of Request:')) IS NOT NULL
      AND EXTRACT(MONTH FROM parse_excel_date(COALESCE(raw_data->>'Date', raw_data->>'Date of Request:'))) = p_month
      AND EXTRACT(YEAR FROM parse_excel_date(COALESCE(raw_data->>'Date', raw_data->>'Date of Request:'))) = p_year
  )
  ...
  -- Debug logging
  RAISE NOTICE 'Excel Monthly Analysis for %/% - Total results: % (Fixed field mappings)', 
