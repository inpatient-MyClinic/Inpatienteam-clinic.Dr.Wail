...
-- View: normalize your uploaded Excel rows
-- Adjust header names if your import uses different labels.
create or replace view public.excel_cases_clean as
select
  id,
  coalesce("Patient's MRN","MRN","Patient MRN")::text           as mrn,
  coalesce("Status","AI")::text                                 as status,
  coalesce("My Clinic Branch","Branch")::text                   as branch,
  parse_excel_date(coalesce("Month","AP","Date"))               as case_date,
  coalesce("Hospital Name","Hospital")::text                    as hospital,
  coalesce("Specialty","Service Specialty")::text               as specialty
from excel_rows_raw
where coalesce("Status","AI") is not null
  and coalesce("Month","AP","Date") is not null;
