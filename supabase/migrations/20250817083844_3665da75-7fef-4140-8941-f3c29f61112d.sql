...
-- Auto-update timestamp trigger (making it immutable)
...
SELECT
  ...
  COALESCE(request_date, created_at::date) as request_date,
  UPPER(NULLIF(branch_code,'')) as branch_code,
  NULLIF(hospital_name,'') as hospital_name,
  NULLIF(specialty,'') as specialty,
...
-- Simple indexes without function expressions
...
CREATE INDEX IF NOT EXISTS ix_excel_created ON excel_requests(created_at);
